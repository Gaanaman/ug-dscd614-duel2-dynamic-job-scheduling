...
Deep Q-Networks (Mnih et al., 2015) fit `Q(s, a)` to `y = r + γ·max_{a'} Q_target(s', a')`, using a
replay buffer to decorrelate samples and a target network to stabilise the regression. The dueling
architecture (Wang et al., 2016) modifies only the head, following the shared trunk with two
streams: one estimates a scalar state value `V(s)`, the other an advantage vector `A(s, a)`. The two
are recombined as
...
separate `Q` per action wastes capacity re-deriving a shared state value. Scheduling has that
...
Their finding on structured state is corroborated elsewhere. Zhang et al. (2020) embed the
disjunctive graph with a GNN to obtain a size-agnostic policy, and Smit et al. (2024) survey the
...
2025). Lv et al. (2025) survey the wider field.
...
`double_q` as a flag rather than silently adopting the stronger method.

## 3. Problem formulation

### 3.1 The scheduling problem

The system has `M` non-preemptive parallel machines, where machine `m` has speed `s_m`, so job `j`
occupies `p_j/s_m` time units. Jobs arrive as a Poisson process with rate `λ`, and an episode
generates `N` of them. Job `j` carries arrival `a_j`, processing time `p_j`, weight `w_j`, deadline
`d_j`, and realised completion `C_j`.
...
exactly 50 epochs, one per job, and `Δt_i = t_{i+1} − t_i` is the elapsed time between them.

### 3.3 State space

The state has fixed dimension `d = 5K + 3M + 4`, so with `K = 10` visible job slots and `M = 5`
machines, `d = 69`. The pending queue is sorted by `(deadline, processing time)` and the first `K`
jobs occupy the observation window. The job window contributes `K` × 5 features: processing time
`/ p_max`; weight `/ w_max`; slack `(d_j − t_i)/H` clipped to `[−1,1]`; waiting time `(t_i − a_j)/H`
clipped to `[0,1]`; and an occupancy flag. The machine bank contributes `M` × 3: time until free
`/ p_max`; speed `/ s_max`; utilisation so far. Four global features complete the vector: queue
length `/ N`; time `/ H`; jobs outstanding `/ N`; arrival rate over capacity.

Every feature is *relational* rather than absolute, with slack and waiting measured against the
...
The direct action space is `Discrete(K · M + 1) = Discrete(51)`. Action `a = k·M + m` assigns the
job in slot `k` to machine `m`, and action `a = K·M` is a no-op that commits no assignment and
advances the clock.

A mask `μ ∈ {0,1}^51` accompanies every observation, with `μ[kM + m] = 1` iff slot `k` is occupied
and machine `m` is idle. Because a decision epoch requires both an idle machine and a pending job,
...
bootstrap target restricts `max_{a'} Q_target(s',a')` to actions legal in `s'`, which requires the
next-state mask in the buffer. The dueling mean is taken over legal actions only, since otherwise
values from unreachable outputs leak into `V(s)`. Finally, the current-state mask must be supplied
during the update, or `Q(s,a)` in the loss differs from what the behaviour policy evaluates, as
Appendix A.2 reports.

Two formulations are implemented and both reported. Formulation A is the direct assignment just
described. Formulation B, the headline, is dispatching-rule selection: `Discrete(8)` over SPT, LPT,
...
Our justification is Han and Yang (2020), and §6.1 gives the mechanism. `FixedRule(SPT)` reproduces
Shortest-Job-First to within 1e-9 on every metric and `FixedRule(FCFS)` reproduces
...
report as `avg_waiting_time × N`. The term is the evaluation objective decomposed over decision
epochs, so the dense per-step signal cannot diverge from the metric it is scored on.
`tests/test_reward.py` asserts the identity numerically on a full episode.

### 3.6 Termination, truncation and discount

Termination occurs when all `N` jobs complete, a genuine absorbing state; truncation occurs at
`T_max = 4N = 200` epochs or when time exceeds `H`. Truncation is a harness limit rather than a
property of the task, so a truncated state is bootstrapped rather than zeroed, which is why the
buffer stores `terminated` alone.

We set `γ = 0.99`. Episodes run 50 decision epochs under every rule, so the effective horizon
...
It does not, and both violations follow from choices we made. Under *queue truncation*, only the
`K = 10` head-of-queue jobs are observable. The queue is sorted by a fixed key so the window holds
the ten most urgent, and `|Q_i|` sits in the global block, so the agent observes the size of what it
cannot see. Under *unobserved future arrivals*, realised arrival times of unreleased jobs are absent
and `λ` is included instead, which makes the process Markov *in distribution*. However the state is
...
The group wrote a custom Gymnasium (Towers et al., 2023) environment implementing the MDP of
...
We fixed the environment constants by measurement rather than by choice. At `λ = 0.55` the makespan
was 97.95 against an arrival bound `N/λ` of 90.9, so the arrival process rather than the scheduler
set the finish time and the rules were indistinguishable. Sweeping `λ` gave 1.0 (`ρ = 1.24`), where
the makespan is 69.06 against a bound of 50.0. Appendix A.3 gives the sweep, and
`scripts/check_load.py` retains the check.
...
entries at a large finite negative value. A finite value rather than `−inf` keeps the loss defined
...
reference (Huang et al., 2022). We wrote it out rather than importing it so that masking could be
threaded everywhere it is required. The settings are Adam at `1 × 10⁻⁴`, batch 128, replay 200,000,
learning starts at 10,000, one gradient step per 4 environment steps, target sync every 1,000,
gradient clipped at 10, and Huber loss. ε decays 1.0 → 0.05 over the first 30% of training, and each
...
(2025); it alters which transitions are drawn, not the learning rule. n-step returns propagate a
...
Round Robin, behind a common `Policy` interface. Each chooses a job and then the fastest idle
...
`RandomMasked` is a diagnostic floor rather than a required baseline: a policy that cannot beat
...
We wrote the protocol into `docs/experimental_protocol.md` before any result existed. It fixes three
...
exploration disabled. All thirteen policies run through the same `harness.run_policy` call on the
...
sufficed, and the seed spread stays at or below 0.062. Training returns include ε = 0.05 and are
...
The best configuration is Formulation B with n-step 3 returns, at −1.235 ± 0.025. It wins on every
...
*when* each rule applies. The measured Q-spread of 0.11 across legal actions, against episode
...
This result does not refute Han and Yang (2020). Their CNN-over-images state, their 85 static
...
supplied an all-ones current-state mask, so `Q(s,a)` during the update differed from what the
...
### 6.4 Why the agent stops short of the best single rule
...
*which rule suits the current queue* while seeing that queue as a flat concatenation of ten slots.
...
Exploration is ε-greedy and uniform over the legal actions, with ε decaying from 1.0 to 0.05 over
...
the return curve is flat over the last 300,000 steps at ε = 0.05.
...
evaluate `double_q` as the D3QN combination the literature favours; and test transfer across
...
sweep and the resulting choice of `λ = 1.0` are in `docs/mdp_spec.md` §10, and the check is retained
...
epoch advances the clock by `Δt = 0` and returns reward 0, while every action that advances time
