...
against all three rules under a protocol fixed before any result was seen. The aims are to specify
the problem as an MDP with a fixed-dimension state, a masked discrete action space and a reward
traceable to the evaluation metrics; to implement and train a Dueling DQN with action masking; to
evaluate it on held-out instances with variation reported across seeds; and to explain the result.
...
This project departs from that literature in one respect. The standard there is D3QN, but the
brief specifies Dueling DQN, so the headline configuration uses dueling alone and exposes
...
The mask is applied in five places. $\epsilon$-greedy exploration samples only
...
The environment constants were set from a load sweep. At \texttt{$\lambda$\ =\ 0.55} the makespan
...
Two enhancements are evaluated by ablation. Prioritised
...
the agent. A baseline with full queue visibility would not face the agent's constraints.
...
The rules also set a second, harder comparison. Under Formulation B the eight rules are themselves
...
\subsection{The dispatching rules}\label{the-dispatching-rules-and-the-bar-they-set}
...
Two rules inside the action set beat all three required baselines. ATC at -1.125 is the reference,
...
Against ATC the agent falls short. ATC scores -1.125 against the agent's -1.235, a difference of
0.109 that exceeds the seed spread. Section 6.4 discusses the cause.

Machine utilisation (0.884--0.927) and makespan vary by under 5\% across all thirteen policies. Both
are dominated by the arrival process, and are reported for completeness.
...
\subsection{Effect of the action space}\label{the-action-space-was-the-binding-constraint}
...
The mechanism lies in the state encoding. SPT implements a comparison across queued jobs.
...
returns near -2, supports this: the network learned state value well and between-action
ordering poorly.
...
\subsection{Faults found during development}\label{three-faults-found-by-measuring-rather-than-assuming}

Three problems appeared during development, none of which raised an error. The action
...
behaviour policy evaluated, and the agent degraded as it trained. Each was found by a diagnostic check run
before the result was accepted. Appendix A gives the forensics and the tests.

\subsection{Why the agent stops short of the best single rule}\label{why-the-agent-stops-short-of-the-best-single-rule}

Two explanations can be excluded. Return plateaus from 400,000 steps with a seed spread under
0.05, so more compute does not close the gap. The reward ranking and the metric
...
Three properties are relevant to deployment. Inference is one forward
