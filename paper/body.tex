...
separate \texttt{Q} per action spends capacity re-deriving a shared state value. Scheduling has that
...
set the finish time and the rules differed by 0.06 in average waiting time. Sweeping \texttt{$\lambda$} gave 1.0 (\texttt{$\rho$\ =\ 1.24}), where
...
\texttt{RandomMasked}, uniform selection among legal actions, is a diagnostic floor rather than a
required baseline, and is reported because it informed the action-space design.
...
sufficed, and the seed spread stays at or below 0.041. Training returns include $\epsilon$ = 0.05 exploration and are therefore lower than the evaluation
numbers.
...
\item
  Prioritised replay leaves the return within the spread: -0.014 against 0.053, with no per-seed
dominance, and -0.052 when combined with n-step.
\end{itemize}
...
makespan vary by under 5\% across policies at this load.}
\label{fig:bars}
\end{figure*}
...
returns near -2, supports this: the between-state signal is the larger of the two by two orders of magnitude.
...
part of the recipe of the closest precedent. In the ablation the change is -0.014 against a combined spread of 0.053, with no per-seed
dominance.

The setting differs from \citet{hanyang2020} in three respects. Their CNN-over-images state, their 85 static
...
\subsection{Validation checks}\label{three-faults-found-by-measuring-rather-than-assuming}

Three checks were run before any result was accepted, and each led to a design change that the
test suite now enforces. A comparison against the uniform-random legal policy showed the no-op
selected at 46.3\% of decision epochs, and the no-op was masked out. An assertion on instance
seeds confirmed that training and held-out bands are disjoint for every seed. A test on the
loss confirmed that \texttt{Q(s,a)} in the update uses the same current-state mask as the
behaviour policy. Appendix A gives the measurements and the tests.
...
arrival times of unreleased jobs are absent. A recurrent encoder, or a short observation history, is the extension for this.

The masked no-op means the policy cannot hold a fast machine for an urgent job arriving
imminently. Potential-based shaping would let the no-op be retained under the present reward,
and is listed under further work.
...
non-stationary arrivals. Transfer to those settings is outside the scope of this study, and the load parameters were
chosen so that the rules separate. With three seeds, differences are compared against the seed
spread.
...
worst case is a suboptimal legal choice rather than an invalid one. Under Formulation B every action is
...
Three validation checks, a uniform-random legal policy, a seed-band assertion and a loss-mask
test, fixed the action space, the instance split and the update before any result was accepted,
and each is retained in the test suite.
